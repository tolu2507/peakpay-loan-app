...
const LoansPage = () => {
  ...
  const { setStatus } = useLoanStore();
  const router = useRouter();
  const userName = (user?.first_name || "") + " " + (user?.last_name || "");

  const [loanFlowData, setLoanFlowDataState] = useState({
    isProcessing: false,
    activeStep: "request"
  });

  const [repaymentData, setRepaymentDataState] = useState({
    view: "initial",
    type: "",
    amount: ""
  });

  const setLoanFlowData = (update: any) => {
    setLoanFlowDataState(prev => ({ ...prev, ...update }));
  };

  const setRepaymentData = (update: any) => {
    setRepaymentDataState(prev => ({ ...prev, ...update }));
  };
...
const LoanRequestView = ({
  ...
  repaymentData: any;
  setRepaymentData: (val: any) => void;
}) => {
  const { loanDetails, setStatus } = useLoanStore();
  const { setModal, isModalVisible } = useGlobalStore();
  const router = useRouter();
  ...
  }, [isModalVisible, setModal, router]);
            ...
            {repaymentData.type === "" ? (
              <div>
                {repaymentData.view === "types" ? (
                  <PaymentTypes repaymentData={repaymentData} setRepaymentData={setRepaymentData} />
                ) : (
                  <PaymentsOptions
...
const PaymentTypes = ({ repaymentData, setRepaymentData }: { repaymentData: any, setRepaymentData: (val: any) => void }) => {
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-[18px] font-semibold text-gray-900">
