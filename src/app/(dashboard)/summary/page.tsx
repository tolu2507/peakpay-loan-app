...
const LoanRequestView = ({ onProceed, summaryFlowData, setSummaryFlowData }: { onProceed: () => void, summaryFlowData: any, setSummaryFlowData: (val: any) => void }) => {
...
const LoansPage = () => {
  ...
  const { setApplied } = useLoanStore();
  const { setModal, isModalVisible } = useGlobalStore();
  const router = useRouter();

  const [summaryFlowData, setSummaryFlowDataState] = useState({
    modalStep: "offer",
    consented: false,
    showSchedule: false,
    pin: ["", "", "", ""],
    rateSelected: { index: -1, rate: "" }
  });

  const setSummaryFlowData = (update: any) => {
    setSummaryFlowDataState(prev => ({ ...prev, ...update }));
  };
  
  const userName = (user?.first_name || "") + " " + (user?.last_name || "");
  const currentStep = summaryFlowData.modalStep;
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex gap-8">
        <div className="flex-1">
          <LoanRequestView 
            onProceed={() => setModal(true)} 
            summaryFlowData={summaryFlowData}
            setSummaryFlowData={setSummaryFlowData}
          />
        </div>
      </div>
      ...
      <Modal isOpen={isModalVisible} onClose={() => setModal(false)}>
        {currentStep === "offer" && (
          <LoanOffer
            consented={summaryFlowData.consented}
        ...
        {currentStep === "congratulations" && (
          <div className="bg-white py-[30px] overflow-hidden px-[20px] rounded-[16px] flex-col flex gap-4 justify-center items-center w-[380px] shadow-2xl">
            ...
            <button
              onClick={() => {
                setApplied("1000000"); // Mock amount for now
                setModal(false);
