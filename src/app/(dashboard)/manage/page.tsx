...
const LoansPage = () => {
  ...
  const [manageFlowData, setManageFlowDataState] = useState({
    pin: ["", "", "", ""],
    showSchedule: false
  });

  const setManageFlowData = (update: any) => {
    setManageFlowDataState(prev => ({ ...prev, ...update }));
  };
      ...
      <div className="flex gap-8">
        <div className="flex-1">
          <LoanManageView 
            ...
            setManageFlowData={setManageFlowData}
          />
        </div>
      </div>
...
const LoanManageView = ({ onProceed, manageFlowData, setManageFlowData }: { onProceed: () => void, manageFlowData: any, setManageFlowData: (val: any) => void }) => {
  const router = useRouter();
  // We'll use useLoanStore for real loan data eventually
  const { appliedAmount } = useLoanStore();
