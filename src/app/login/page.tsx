...
import { useLoginStore } from "@/web-portable/store/useLoginStore";
...
export default function LoginPage() {
  ...
  const { resetForm: resetSignupForm } = useSignupStore();
  const { 
    email, 
    password, 
    setEmail, 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await login();
      // Clear onboarding data if any
      resetSignupForm();
      router.push("/dashboard");
    } catch (err) {
      // Error is handled by the store and global store
